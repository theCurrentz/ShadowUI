...
ShadowUI is an opinionated World of Warcraft UI addon for action bars, interface chrome, and cast bars. It supports Classic Era only.

## Installation

Copy the addon directory to `Interface/AddOns/ShadowUI`, then enable ShadowUI from the character selection screen. On first apply each session, ShadowUI turns Auto Loot on for that Character. Combined bags need **Bagnon** and **Bagnon Bank** enabled; ShadowUI skins those windows and applies the Rainbow Organizer.
...
- `/shadowui deck` replaces the Warrior Action Deck Action Slots (out of combat).
...
Terms: [CONTEXT.md](CONTEXT.md). Module layout and resolve rules: [docs/architecture.md](docs/architecture.md). Classic Era macros: [docs/macros/](docs/macros/). Sidecar rolodex: `pnpm --dir macro-cursor dev`.

## Options

Open with `/shadowui` or `/sui`. The panel covers variants, talent-tree binding, Action Slot hard lock, an on/off toggle for every Bar including pet and possess, edit-layer selection, Layout Edit Mode, Keybind Edit Mode, Place Action Deck, and resets. It does not contain bar-layout sliders. A Bar toggle writes `enabled` to the selected Layer. Toggles stay shown when the current class Layout has no entry.
...
`deck_spec` covers the Warrior Action Deck: stance-page maps, preflight macro creation, managed-slot clearing, duplicate names, and placement.
...
`button_skin_spec` asserts a 0.05 chrome fill, a 2px icon inset, and a 4px outer edge on bound action buttons, and that an empty Action Slot stays hidden with no Darken fill and no Keybind label.
...
`auras_spec` asserts buff chrome is darkest, unused slots stay empty, player buffs sit 4px below the top of the screen and 4px left of the square minimap, and debuff type colour stays native.
...
`target_spellbar_spec` asserts Target of Target stays on the Blizzard default offset and the Target Frame spell bar nests under the mana bar with a remaining / duration count.
...
`statusbars_spec` asserts Meter Fill is a horizontal lighting overlay on health, mana, rage, Name Background, and nameplate health, and that StatusBar vertex colour cannot flatten it.
...
`rainbow_spec` asserts bag items group by category (Hearthstone, mounts, fixtures, gear, consumables, materials, quest, junk), not by bag, and that category glow does not replace quality chrome.
`bagnon_spec` asserts Bagnon inventory and bank get Darken fill, Outer Edge, search, sort, no bag breaks, and the Rainbow Organizer. Missing Bagnon is a no-op.
...
- [ ] **Chrome** — 0.05 icon chrome and Outer Edge on bound action buttons and Stance Bar buttons; empty Action Slots stay hidden, including Keybind labels; square minimap flush to top-right in a 16px Darken buffer at 0.6 alpha with Zone Text on top, World Layer on the bottom, Blizzard Time under the map, mouse-wheel zoom, and an Outer Edge; parked Currentz chat; micro menu + one bag button, bottom-right, one row
...
- [ ] **Meter Fill** — player and target health and power bars, Name Background, and nameplate health bars use a horizontal lighting overlay of the live colour
...
- [ ] **Bagnon** — with Bagnon and Bagnon Bank loaded, inventory and bank are near-black with Outer Edge; search and sort work; items group by category with a coloured glow; quality borders stay; groups are not bags
...
- [ ] **Warrior Action Deck** — out of combat, `/shadowui deck` (Fury) puts Hamstring on 1, Charge/Intercept on 2, Cleave (not Cannibalize or Cone of Cold) on 3, the smart interrupt on 4, Bloodthirst on 73/85/97, and stances on 109–111; empty managed positions are clear; slot 13 is unchanged; missing macros appear on the General tab
...
- `/shadowui deck` clears and replaces Warrior slots 1–12 and 73–111 for the active Variant. It leaves other Action Slots unchanged. Every deck entry must match its required Warrior body marker; cross-class short names such as `c`, `ms`, `ls`, and `cb` cannot select another class's macro.
